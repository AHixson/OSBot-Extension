class ScriptTable extends Template {
	...
	validate() {
		...
		if (b) {
			this.reloadButtonElement = this.element.querySelector('input[name="refresh-script-overview"]');
			this.saveTrialDurations = this.element.querySelector('input[name="save-trial-durations"]');
			...
			this.footerElements = {
				...
				'sales-profit': this.footerElement.querySelector('td#all-sales-profit a'),
				'sales-latest': this.footerElement.querySelector('td#all-sales-latest a'),
				'sales-renewals': this.footerElement.querySelector('td#all-sales-renewals a'),
				'trials-total': this.footerElement.querySelector('td#all-trial-total a'),
				'trials-active': this.footerElement.querySelector('td#all-trial-active a'),
				'trials-expired': this.footerElement.querySelector('td#all-trial-expired a'),
			};
			// Listeners
	...
	getAllRows(main = true) {
		return Array.from(this.bodyElement.querySelectorAll(`tr[data-script-id].${main ? 'main' : 'sub'}-row`));
	}
	
	getRow(scriptId, main = true) {
		return this.bodyElement.querySelector(`tr[data-script-id="${scriptId}"].${main ? 'main' : 'sub'}-row`);
	}
	
	getRowByNexusId(nexusId, main = true) {
		return this.bodyElement.querySelector(`tr[data-nexus-id="${nexusId}"].${main ? 'main' : 'sub'}-row`);
	}
	...
	addScript(script) {
		this.rowTemplate.appendTo(this.bodyElement, {
			...
			'script\\.lastCompiled': (script.lastCompiledDate ? script.lastCompiledDate.toLocaleDateString() : ''),
			...
			'trial\\.trialTime': (localStorage.getItem(`trial-time-${script.scriptId}`) || 60)
		});
	}
	...
	addTrial(scriptId, trialRecords) {
		...
		if (row) {
			...
			trialActiveCount = trialRecords.filter(trialRecord => !trialRecord.expired).length;
			...
			row.querySelector('td[data-id="trials-active"] a').innerText = trialActiveCount;
			row.querySelector('td[data-id="trials-expired"] a').innerText = trialExpiredCount;
		}
	}
	...
	addSalesSummary(nexusId, salesSummary) {
		var row = this.bodyElement.querySelector(`tr[data-nexus-id="${nexusId}"].main-row`);
		var cell = null
		if (row) {
	...
	addTransactionRecords(nexusId, transactions) {
		...
		if (row && transactions && transactions.length > 0) {
			...
			cell.innerHTML = transactions.filter(r => !r.renewal)
	...
	addAllTransactionRecords(scripts) {
		for (let scriptId in scripts) {
			...
			Sales.get(salesUrlCode)
				 .then(transactions => this.addTransactionRecords(nexusId, transactions));
		}
	}
	...
	calculateAndUpdateFooter() {
		for (let cellId in this.footerElements) {
			let cells = Array.from(this.bodyElement.querySelectorAll(`tr.main-row td[data-id="${cellId}"] a`));
			let num = cells.map(cell => cell.innerText)
						   .map(parseFloat)
						   .filter(Boolean)
						   .reduce((a, b) => a + b, 0);
	...
	saveAllTrialDurations() {
		Array.from(this.bodyElement.querySelectorAll('input[name^="trial-time-"]')).forEach(input => this.saveTrialDuration(input));
	}
}