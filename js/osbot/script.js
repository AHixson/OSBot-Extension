...
const Script = (function() {
	...
	function extractScriptInformation(row) {
		...
		scriptThreadShortUrl = (scriptThreadUrl.endsWith('/') ? scriptThreadUrl.slice(0, -1) : scriptThreadUrl).split('/').pop();
		// Nexus ID
		...
		// Sales URL
		if (premium) {
			salesUrl = extractUnixPath(cells[4].querySelector('button').getAttribute('onclick'));
			salesUrlCode = extractUrlCode(salesUrl);
		}
		// Compile date
		lastCompiledDate = cells[5].innerText;
		lastCompiledDate = (lastCompiledDate === 'null' ? '' : new Date(lastCompiledDate));
		// Git/reverse
		gitCommitHash = cells[6].innerText;
		reverseStacktraceUrl = extractUnixPath(cells[7].querySelector('button').getAttribute('onclick'));
